...
\begin{frame}
  ...
  \begin{block}{Nota Bene (N.B.)}
    Random samples naturally lead to \textbf{sampling variability} -- the fact that characteristics of interest will vary from sample to sample.
  \end{block}
\end{frame}
...
\begin{frame}
  ...
  \begin{block}{numerical}
    Numerical data is quantitative; e.g.\ it takes on numerical values and all mathematical operations $(+, -, *, /, <, >, =, \ldots)$ make sense with these values.
  \end{block}
\end{frame}
...
\begin{frame}[fragile]
  \frametitle{Data Types in \texttt{R}}
  \begin{block}{categorical}
    \texttt{R} calls a categorical variable a factor, and maintains the word \textbf{levels} to mean the values the factor can take on.
  \end{block}
...
The \textbf{sample proportion} summarizes multiple observations of a categorical variable; divide the number of ``successes'' by the number of observations:
...
\end{frame}
 ...
 \begin{frame}
   \frametitle{Median}
The \textbf{sample median} gives a (different) summary of the middle of multiple observations of a numeric variable.  There is no simple mathematical expression for it.  Hence, we think of the median as the number in the middle of the ordered observations, dependent on the sample size $n$:
...
The most common measures of spread are the \textbf{variance} and the \textbf{standard deviation}.  Think of the variance as the average squared distance away from the mean.  The standard deviation is the square root of the variance, which essentially gives us interpretable units (no longer squared).
\end{frame}
...
\begin{frame}[fragile]
  \frametitle{Sample Standard Deviation, in \texttt{R}}
  \texttt{R} will happily do calculate the $s$, the sample standard deviation, for you.
