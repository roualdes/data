...
A \textbf{discrete random variable}, being a function, is often (but not always) described using a table.  Such tables denote the values the random variable can take on and the probability of each value.  Consider the random variable $X$ (capitlized) that takes on the following values $x$ (lower case).
