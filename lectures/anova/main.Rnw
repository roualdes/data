...
msleep %>%
    qplot(vore, sleep_rem, data=., # LHS piped to .
...
\begin{frame}[fragile]
  \frametitle{ANOVA, Lite Example}
  But it's almost too easy -- where's the devil?
<<>>=
fit <- anova(lm(sleep_rem ~ factor(vore), data=msleep))
#        y (numerical) ~ x (categorical)
fit[,"Pr(>F)"] # get p-value
@
\end{frame}
...
\begin{frame}[fragile]
  ...
  Suppose you chose a level of significance of $\alpha = 0.05$.  The probability of observing at least one significant result, out of the $28$ tests, just by chance is
<<echo=FALSE>>=
sum(dbinom(1:28, 28, 0.05))
@
\end{frame}
...
suppressMessages({library(ape)
    data(carnivora)})
p <- qplot(Family, SB, data=carnivora, geom="boxplot",
           ylab="Brain weight (g)")
@
\end{frame}
...
\begin{frame}[fragile]
  \frametitle{\texttt{ape::carnivora}, average brain weight}
We of course first plot the data $\Rightarrow$ check assumptions.
<<echo=FALSE, fig.width=6, fig.height=4, fig.align="center">>=
p
@
\end{frame}
...
\begin{frame}[fragile]
  \frametitle{\texttt{ape::carnivora}, average brain weight}
  Fit the model, following guide of assumptions.
<<>>=
carnivs <- filter(carnivora,
                  !(Family %in% c("Ailuridae",
                                  "Procyonidae",
                                  "Viverridae")))
fit <- anova(lm(SB~Family, data=carnivs))
@
\end{frame}
