...
\begin{frame}[fragile]
  ...
  <<results="hide">>=
  model <- lm(salary\textasciitildeposition, data=mlb)
  anova(model)
  @
\end{frame}
...
\begin{frame}[fragile]
  ...
  Fit the model.
  <<>>=
  fit <- lm(SB\textasciitildeFamily, data=carnivs)
  anova(fit)
@ 
\end{frame}
...
\begin{frame}[fragile]
  ...
  t1 <- t.test(SB\textasciitildeFamily, data=carns1,
                 conf.level=1-astar)
  @
\end{frame}
...
\begin{frame}[fragile]
  ...
  t2 <- t.test(SB\textasciitildeFamily, data=carns2,
                 conf.leve=1-astar)
  @
\end{frame}
...
\begin{frame}[fragile]
  ...
  Then ran ANOVA.
<<results="hide">>=
mod <- lm(SB\textasciitildeFamily, data=carnivs)
anova(mod)
@ 
\end{frame}
