...
\section{Decision Making is hard}
\begin{frame}
  \frametitle{Hypothesis Testing, the decision}
We conclude a formal hypothesis test by making a decision between $H_0$ and $H_1$.  But did we decide correctly?
\end{frame}

\begin{frame}
  \frametitle{Hypothesis Testing, decision errors}
...
\begin{frame}
  ...
  \begin{itemize}
  \item<2-> Lower Type 2 Error by convicting more (guilty) people.
  \item<3-> What then of wrongful convictions, i.e.\ Type 1 Error rates?
  \end{itemize}
\end{frame}
