...
\begin{frame}
  \frametitle{Conditional Probability, online interactive explanation}
  \href{http://setosa.io/conditional/}{Visual Explanations Blog: conditional probability}
\end{frame}
...
\item<1-> Know
  ...
  \item<1-> probability second light is green, given first light is red, $P(B|A^c) = 0.09$
  \end{itemize}
\item<2-> Want
  \begin{itemize}
  \item<2-> $P(B)$
...
\begin{frame}
  ...
  The intuition of independence is best seen mathematically via conditional probability,
...
Two events $A$ and $B$ are said to be \textbf{pairwise independent} if
  \[ P(A \cap B) = P(A)P(B). \]
\end{block}
\end{frame}
...
\begin{frame}
  ...
  Randomly draw a card from a standard $52$ card deck.  Let $A$ be the event that the card is a five and $B$ be the event that the card is a spade.  Are the events $A$ and $B$ independent?

  \pause $P(A \cap B) = 1/52$.  Next $P(A) = 4/52$ and $P(B) = 13/52$ gives $P(A)P(B) = 1/52$.  Therefore $P(A \cap B) = P(A)P(B)$, so the events are independent.
\end{frame}

\subsection{mutual}

\begin{frame}
  \frametitle{Independence, take $2$}

  Independence over more than two sets is referred to as \textbf{mutual independence}.

  \begin{block}{mutual independence}
    Events $E_1, \ldots, E_n$ are mutually independent if for every subset $E_{i_1}, \ldots E_{i_k}, k \leq n$,
    \[ P(\cap_{l=1}^k E_{i_l}) = \prod_{l=1}^k P(E_{i_l}). \]
  \end{block}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
  \item<1-> It is through probability that much of statistics is justified.
  \item<2-> For arbitrary events
    \begin{itemize}
    \item<2-> now $E = \{\text{encounter green light}\}$
    \item<3-> soon $E = \{X > 3\}$
    \end{itemize}
  \item<4-> Bayes' Theorem, though simple is the source of paradigm shifting Bayesian Statistics
  \end{itemize}
\end{frame}
