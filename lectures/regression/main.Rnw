...
<<echo=FALSE, fig.align="center", fig.height=2.5, fig.width=2.5, >>=
...
\begin{itemize}
\item<2-> intercept -- when family income is $0$, average gift aid is expected to be $24.32$ thousands of dollars.
\item<3-> slope -- for every $\$1000$ increase in family income, average gift aid is expected to decrease by -$\$43.07$.
\end{itemize}
\end{frame}
