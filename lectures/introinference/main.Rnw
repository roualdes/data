...
\begin{frame}
  ...
  Since $\bar{X}$ consists of $n$ random variables $X_1, \ldots, X_n$, we think of $\bar{X}$ itself as a random variable.  The population distribution for $\bar{X}$, from which any sampled value of $\bar{x}$ is drawn, is often unkown, but reasonably well approximated by the normal distribution due the Central Limit Theorem\footnote{Some, quite plausible, assumptions must be met for the CLT to apply, but in practice these assumptions are often are met.}.
\end{frame}
...
\begin{frame}
  ...
  Our net is called a \textbf{confidence interval}, and we make a comprimise between width and confidence.

  \begin{block}{confidence interval}
    A confidence interval is an interval $[ \hat{\theta}_L, \hat{\theta}_U ]$ computed from a samlpe,  used to estimate a population parameter of interest.
  \end{block}
\end{frame}
