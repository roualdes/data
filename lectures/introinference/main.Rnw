...
\begin{frame}
  ...
  $\bar{X}$ consists of and therefore is itself as a random variable.  The sampling distribution for $\bar{X}$ is often unknown, but reasonably well approximated by the normal distribution due the Central Limit Theorem\footnote{Some, quite plausible, assumptions must be met for the CLT to apply, but in practice these assumptions are often met.}.
\end{frame}

\section{Confidence Intervals}
\begin{frame}
  \frametitle{Towards Interval Estimation}
  The hope is that an interval, rather than a point estimate, will better capture the true parameter of interest: \href{https://roualdes.us/misc/garfield.jpg}{Garfield}.
\end{frame}
...
\begin{frame}
  \frametitle{Confidence Interval, example (to convey idea)}
  A sample from the Youth Risk Behavioral Surveillance System (YRBSS) suggests the average student height is $\bar{X} = 1.697$ meters.  Construct a confidence interval.
...
\begin{frame}[fragile]
  \frametitle{Confidence Interval, a trade off}
...
\begin{frame}
  \frametitle{Confidence Intervals, margin of error}

To create an interval we add and subtract some amount from $\bar{X}$, this amount is the \textbf{margin of error}.

  \begin{block}{margin of error}
  In a confidence interval, $z^* \cdot \sigma_{\bar{X}}$ is called the margin of error.
  \end{block}
\end{frame}

\begin{frame}[fragile]
  \frametitle{Confidence Interval, sample mean}
  Assume $\bar{X} \sim N(\mu, \sigma^2_{\bar{X}})$ such that $\sigma^2_{\bar{X}} = Var(\bar{X}) = \frac{\sigma^2}{n}$.  A confidence interval\footnote{If $\alpha = 0.05$, then a $95\%$ confidence interval will use $z^* = qnorm(0.975) = \Sexpr{qnorm(0.975)}$.} is constructed as
...
\begin{frame}
  \frametitle{Confidence Interval, example}
    A sample, $n=100$, from the Youth Risk Behavioral Surveillance System (YRBSS) suggests the average student height is $\bar{X} = 1.697$ meters with a standard deviation of $\sigma = 0.088$ meters.  What is an approximate $99$\% confidence interval for the average height of all of the YRBSS students?

\only<2->{
    \[ [1.697 - 2.58 \cdot 0.088/\sqrt{100}, 1.697 + 2.58 \cdot 0.088/\sqrt{100}] \]
}
\end{frame}
...
xbar <- 1.697
se <- 0.0088
xbar + qnorm(c(0.005, 0.995))*0.0088 # unpack this
@
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
  \item<2-> Establish  hypothesis, null $(H_0)$ and alternative $(H_1)$.
    \begin{itemize}
    \item<2-> $H_0$ and $H_1$ are statements about population parameters
    \end{itemize}

  \item<3-> Collect data.
  \item<4-> Analyze data (calculate summary statistics) -- we'll most often be estimating $\mu$ -- and \textbf{p-value}.
  \item<5-> Make conclusion by comparing p-value to \textbf{level of significance}.
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  The \textbf{p-value} helps us decide between $H_0$ and $H_1$.
...
We define a largest level at which we are willing to incorrectly conclude.  We call this value the level of significance, and give it the symbol $\alpha$\footnote{Common values of $\alpha$ are $0.05$ and $0.01$.}.
...
n <- 1014
phat <- 984/n
sigma <- sqrt(phat*(1-phat)) # vector x? => use sd(x)
