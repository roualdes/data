...
\begin{frame}
  ...
  There are four assumptions about linear regression,
  \begin{itemize}
  ...
  \item<4-> Constant Variability -- variation of points around least squares line remains roughly constant, and
  \item<5-> Normality -- the residuals should be nearly normal,
  \end{itemize}
\pause  three of which are easy to check.
\end{frame}
