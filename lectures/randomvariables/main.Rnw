...
\begin{frame}
  ...
  We theorize that data are generated from a function that only takes on mass at each value in the sample space.  Probability statements are made from area under the curve.
\end{frame}
