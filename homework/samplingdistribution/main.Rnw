...
Due: 2016-03-12 \\
\begin{enumerate}
\item Assume $X_1, \ldots, X_n \sim_{iid} G$ where $Var(X) = \sigma^2$.
  \begin{enumerate}
  \item Find the expected value of $\hat{\sigma}^2$.
  \item How can you modify the estimator $\hat{\sigma}^2$ so that is is unbiased?  Write down the new estimator.
  \end{enumerate}
